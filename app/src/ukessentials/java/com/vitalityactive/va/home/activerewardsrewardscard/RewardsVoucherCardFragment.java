...
public class RewardsVoucherCardFragment extends HomeScreenCardFragment {
    ...
    protected void configureView(View view) {
        if (card != null) {
            ...
            if (RewardId._STARBUCKSVOUCHER == getRewardId()) {
                switch (card.status) {
                    case AVAILABLE:
                        title.setText(card.rewardValueDesc);
                        action.setText(R.string.card_action_title_use_reward1093);
                        break;
                    case NOT_ISSUED:
                        title.setText(R.string.card_title_starbucks_free_drink_not_awarded1096);
                        title.setLines(2);
                        subTitle.setVisibility(View.GONE);
                        break;
                    case CONFIRM_DETAILS:
                        title.setText(R.string.card_title_confirm_details1091);
                        break;
                    case PENDING:
                        title.setText(card.rewardValueDesc);
                        subTitle.setText(R.string.card_title_starbucks_free_drink_pending1095);
                        break;
                }
            } else if (card.status == HomeCardType.StatusType.AVAILABLE) {

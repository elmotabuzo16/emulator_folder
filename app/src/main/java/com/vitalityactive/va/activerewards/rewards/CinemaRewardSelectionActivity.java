...
public class CinemaRewardSelectionActivity
        extends BasePresentedActivity<CinemaRewardSelectionPresenter.UserInterface, CinemaRewardSelectionPresenter>
        implements CinemaRewardSelectionPresenter.UserInterface, EventListener<AlertDialogFragment.DismissedEvent>,
        RewardSelectionViewHolder.OnRewardSelectionToggledListener {

    public static final String REWARD_UNIQUE_ID = "REWARD_UNIQUE_ID";
    private static final String CINEMA_SELECTION_CONFIRMATION_ALERT = "CINEMA_SELECTION_CONFIRMATION_ALERT";

    @Inject
    EventDispatcher eventDispatcher;

    @Inject
    CinemaRewardSelectionPresenter cinemaRewardSelectionPresenter;
    ...
    private RewardSelectionDTO selectedRewardVoucher;
    ...
    protected void create(@Nullable Bundle savedInstanceState) {
        ...
        getPresenter().setRewardUniqueId(getIntent().getLongExtra(REWARD_UNIQUE_ID, 0L));
    ...
    public void onEvent(AlertDialogFragment.DismissedEvent event) {
        if (positiveButtonTapped(event)) {
            getPresenter().onConfirm(selectedRewardVoucher);
        }
    }

    private void retryVoucherRequest() {
        if (selectedRewardVoucher != null) {
            getPresenter().onConfirm(selectedRewardVoucher);
        } else {
    ...
    @Override
    protected CinemaRewardSelectionPresenter.UserInterface getUserInterface() {
        return this;
    }

    @Override
    protected CinemaRewardSelectionPresenter getPresenter() {
        return cinemaRewardSelectionPresenter;
    }
    ...
    public void handleChooseButtonTapped(View view) {
        AlertDialogFragment.create(
                CINEMA_SELECTION_CONFIRMATION_ALERT,
                getString(R.string.AR_rewards_cinema_selection_dialog_title_1116),
                getString(R.string.AR_rewards_cinema_selection_dialog_description_1117),
                getString(R.string.cancel_button_title_24),
                null,
                getString(R.string.confirm_title_button_182))
                .show(getSupportFragmentManager(), CINEMA_SELECTION_CONFIRMATION_ALERT);
    }
    ...
    public void navigateAfterRewardVoucherSelected(long uniqueId) {
        navigationCoordinator.navigateAfterSpecificCinemaSelection(this, uniqueId);
    }
